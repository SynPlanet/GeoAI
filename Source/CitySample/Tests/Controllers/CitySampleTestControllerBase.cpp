...
namespace CitySampleTest
{
	...
	static TAutoConsoleVariable<bool> CVarRestartPieEachIteration(
		TEXT("CitySampleTest.RestartPieEachIteration"),
		true,
		TEXT("If true and running in PIE, the PIE session will be restarted for each test run"),
		ECVF_Default);
	...
	static TAutoConsoleVariable<float> CVarSecondsBetweenIterationsPIE(
		TEXT("CitySampleTest.SecondsBetweenIterationsPIE"),
		5.f,
		TEXT("Delay time between PIE sessions when using RestartPieEachIteration"),
		ECVF_Default);
}
...
void UCitySampleTestControllerBase::OnInit()
{
	...
	bRestartPieEachIteration = CitySampleTest::CVarRestartPieEachIteration.GetValueOnGameThread();

	double RequestedSecondsBetweenIterations = CitySampleTest::CVarSecondsBetweenIterationsPIE.GetValueOnGameThread();
	if (RequestedSecondsBetweenIterations >= 0)
	{ 
		bSecondsBetweenIterationsPIE = RequestedSecondsBetweenIterations;
	}
