...
void ACitySampleVehicleBase::HandbrakeActionPressedBinding(const struct FInputActionValue& ActionValue)
{
	if (bInputsActive)
	{
...
void ACitySampleVehicleBase::HandbrakeActionReleasedBinding(const struct FInputActionValue& ActionValue)
{
	if (bInputsActive)
	{
		SetHandbrakeInput(false);
...
void ACitySampleVehicleBase::UpdateTransformsOverTime(USkeletalMeshComponent* SKM_Proxy)
{
	if (SKM_Proxy)
	{
		...
		TransformsOverTime.Pop(EAllowShrinking::No);
	}
}
